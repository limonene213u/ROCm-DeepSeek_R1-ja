...
本研究では、最近リリースされたDeepSeek R1モデルの日本語特化適応について、言語学的特徴を活用したデータ拡張技術とAMD MI300Xハードウェア最適化を組み合わせた手法を提案する。DeepSeek R1の推論能力を保持しながら、日本語の複雑な言語学的特徴（形態素解析、敬語システム、語順柔軟性）に対応した効率的な学習システムを構築した。実験結果により、提案手法が既存の日本語言語モデルと比較して優れた性能を示すことを実証した。

**キーワード：** 大規模言語モデル、日本語自然言語処理、LoRA、形態素解析、MI300X最適化
